...
class LSV07I_Ajax_Fitness {

    public static function init() {
        $actions = [
            'lsv07i_fit_get_data',
            'lsv07i_fit_get_slots',
            'lsv07i_fit_save_slot',
            'lsv07i_fit_delete_slot',
            'lsv07i_fit_anw_get_or_create',
            'lsv07i_fit_anw_save_eintrag',
            'lsv07i_fit_anw_trainer_abr',
            'lsv07i_fit_anw_mark_ausfall',
            'lsv07i_fit_anw_get_sessions',
            'lsv07i_fit_save_gruppe',
            'lsv07i_fit_delete_gruppe',
            'lsv07i_fit_get_sportler',
            'lsv07i_fit_save_sportler',
            'lsv07i_fit_delete_sportler',
            'lsv07i_fit_get_trainer',
            'lsv07i_fit_save_trainer',
            'lsv07i_fit_delete_trainer',
        ];
        foreach ( $actions as $action ) {
            add_action( 'wp_ajax_' . $action, [ __CLASS__, str_replace( 'lsv07i_fit_', '', $action ) ] );
        }
    }
    ...
    public static function get_data() {
        ...
        $gruppen = $wpdb->get_results(
            "SELECT g.*, COUNT(sg.sportler_id) AS sportler_anzahl
               FROM {$p}lsv07i_fit_gruppen g
          LEFT JOIN {$p}lsv07i_fit_sportler_gruppe sg ON sg.gruppe_id = g.id
        ...
        $trainer = $wpdb->get_results(
            "SELECT t.*, GROUP_CONCAT(tg.gruppe_id) AS gruppe_ids
               FROM {$p}lsv07i_fit_trainer t
          LEFT JOIN {$p}lsv07i_fit_trainer_gruppe tg ON tg.trainer_id = t.id
              WHERE t.aktiv = 1
        ...
        $slots = $wpdb->get_results(
            "SELECT s.*, g.name AS gruppe_name
               FROM {$p}lsv07i_fit_slots s
          LEFT JOIN {$p}lsv07i_fit_gruppen g ON g.id = s.gruppe_id
    ...
    public static function get_sportler() {
        ...
        if ( $gruppe_id ) {
            $rows = $wpdb->get_results( $wpdb->prepare(
                "SELECT s.*
                   FROM {$p}lsv07i_fit_sportler s
                   JOIN {$p}lsv07i_fit_sportler_gruppe sg ON sg.sportler_id = s.id AND sg.gruppe_id = %d
                  WHERE s.aktiv = 1
            ...
            $rows = $wpdb->get_results(
                "SELECT s.* FROM {$p}lsv07i_fit_sportler s WHERE s.aktiv = 1
               ORDER BY s.last_name ASC, s.first_name ASC",
                ARRAY_A
            );
        }
        ...
        if ( ! empty( $rows ) ) {
            ...
            if ( ! empty( $sportler_ids ) ) {
                ...
                // Alle Zuordnungen Sportler→Gruppe
                $sg_rows = $wpdb->get_results(
                    "SELECT sportler_id, gruppe_id FROM {$p}lsv07i_fit_sportler_gruppe
    ...
    public static function save_sportler() {
        LSV07I_Access::check( 'fitnesswart' );
        global $wpdb;
        $p          = $wpdb->prefix;
        ...
        if ( $id ) {
            $wpdb->update( $p . 'lsv07i_fit_sportler', $data, [ 'id' => $id ], $fmt, [ '%d' ] );
        } else {
            $wpdb->insert( $p . 'lsv07i_fit_sportler', $data, $fmt );
            $id = $wpdb->insert_id;
        }

        // Gruppen-Zuordnung aktualisieren
        $wpdb->delete( $p . 'lsv07i_fit_sportler_gruppe', [ 'sportler_id' => $id ], [ '%d' ] );
        foreach ( $gruppe_ids as $gid ) {
            $wpdb->insert( $p . 'lsv07i_fit_sportler_gruppe',
    ...
    public static function delete_sportler() {
        ...
        $id = absint( $_POST['id'] ?? 0 );
        $wpdb->delete( $p . 'lsv07i_fit_sportler_gruppe', [ 'sportler_id' => $id ], [ '%d' ] );
        $wpdb->delete( $p . 'lsv07i_fit_sportler',        [ 'id'          => $id ], [ '%d' ] );
    ...
    public static function save_gruppe() {
        ...
        if ( $id ) {
            $wpdb->update( $p . 'lsv07i_fit_gruppen',
                [ 'name' => $name, 'sort_order' => $sort_order ],
                [ 'id'   => $id ], [ '%s', '%d' ], [ '%d' ]
            );
        } else {
            $wpdb->insert( $p . 'lsv07i_fit_gruppen',
    ...
    public static function delete_gruppe() {
        ...
        $p  = $wpdb->prefix;
        $id = absint( $_POST['id'] ?? 0 );
        $wpdb->delete( $p . 'lsv07i_fit_sportler_gruppe', [ 'gruppe_id' => $id ], [ '%d' ] );
        $wpdb->delete( $p . 'lsv07i_fit_trainer_gruppe',  [ 'gruppe_id' => $id ], [ '%d' ] );
        $wpdb->delete( $p . 'lsv07i_fit_gruppen',         [ 'id'        => $id ], [ '%d' ] );
        wp_send_json_success();
    }
    ...
    public static function save_trainer() {
        ...
        if ( $id ) {
            $wpdb->update( $p . 'lsv07i_fit_trainer', $data, [ 'id' => $id ], $fmt, [ '%d' ] );
        } else {
            $wpdb->insert( $p . 'lsv07i_fit_trainer', $data, $fmt );
            $id = $wpdb->insert_id;
        }

        $wpdb->delete( $p . 'lsv07i_fit_trainer_gruppe', [ 'trainer_id' => $id ], [ '%d' ] );
        foreach ( $gruppe_ids as $gid ) {
            $wpdb->insert( $p . 'lsv07i_fit_trainer_gruppe',
    ...
    public static function delete_trainer() {
        ...
        $id = absint( $_POST['id'] ?? 0 );
        $wpdb->delete( $p . 'lsv07i_fit_trainer_gruppe', [ 'trainer_id' => $id ], [ '%d' ] );
        $wpdb->delete( $p . 'lsv07i_fit_trainer',        [ 'id'         => $id ], [ '%d' ] );
        wp_send_json_success();
    }

    // ── Fitness Trainingszeiten ────────────────────────────────────────────
    ...
    public static function save_slot() {
        ...
        if ( $id ) {
            $wpdb->update( $p . 'lsv07i_fit_slots', $data, [ 'id' => $id ], $fmt, [ '%d' ] );
        } else {
            $wpdb->insert( $p . 'lsv07i_fit_slots', $data, $fmt );
            $id = $wpdb->insert_id;
        }
    ...
    public static function delete_slot() {
        ...
        $p  = $wpdb->prefix;
        $id = absint( $_POST['id'] ?? 0 );
        $wpdb->delete( $p . 'lsv07i_fit_slots', [ 'id' => $id ], [ '%d' ] );
        $slots = $wpdb->get_results(
            "SELECT s.*, g.name AS gruppe_name FROM {$p}lsv07i_fit_slots s
          LEFT JOIN {$p}lsv07i_fit_gruppen g ON g.id = s.gruppe_id
    ...
    public static function anw_get_or_create() {
        LSV07I_Access::check( 'fit_intern' );
        global $wpdb;
        $p       = $wpdb->prefix;
        ...
        $slot = $wpdb->get_row( $wpdb->prepare(
            "SELECT s.*, g.name AS gruppe_name
               FROM {$p}lsv07i_fit_slots s
               JOIN {$p}lsv07i_fit_gruppen g ON g.id = s.gruppe_id
        ...
        $session = $wpdb->get_row( $wpdb->prepare(
            "SELECT a.*, g.name AS gruppe_name
               ...
               JOIN {$p}lsv07i_fit_gruppen g ON g.id = a.gruppe_id
              WHERE a.slot_id = %d AND a.training_datum = %s LIMIT 1",
            $slot_id, $datum
        ), ARRAY_A );

        if ( ! $session ) {
            $wpdb->insert( $p . 'lsv07i_fit_anwesenheit', [
                'gruppe_id'      => $gruppe_id,
                'slot_id'        => $slot_id,
        ...
        $eintraege = $wpdb->get_results( $wpdb->prepare(
            "SELECT * FROM {$p}lsv07i_fit_anwesenheit_eintraege WHERE anwesenheit_id = %d",
            $session['id']
        ), ARRAY_A );
    ...
    public static function anw_save_eintrag() {
        ...
        $wpdb->query( $wpdb->prepare(
            "INSERT INTO {$p}lsv07i_fit_anwesenheit_eintraege (anwesenheit_id, sportler_id, status)
             VALUES (%d, %d, %s)
    ...
    /**
     * Fitness-Trainer-Abrechnung automatisch befüllen wenn Anwesenheit gespeichert wird.
    ...
    public static function anw_trainer_abr() {
        ...
        // Fit-Trainer-ID des aktuellen Nutzers ermitteln
        $tri_trainer_id = LSV07I_Access::get_fit_trainer_id();
        if ( ! $tri_trainer_id ) {
            // Kein Fit-Trainer-Profil → kein Eintrag nötig
        ...
        // Fitness-Abrechnung holen oder anlegen (bereich = 'fitness')
        $abr = $wpdb->get_row( $wpdb->prepare(
            "SELECT id FROM {$p}lsv07i_abrechnung
              WHERE trainer_id = %d AND bereich = 'fitness' AND quartal = %s AND jahr = %d LIMIT 1",
        ...
        $exists = $wpdb->get_var( $wpdb->prepare(
            "SELECT id FROM {$p}lsv07i_abr_trainingstage
              WHERE abrechnung_id = %d AND datum = %s AND quelle = 'fitness' LIMIT 1",
        ...
        if ( ! $exists ) {
            $wpdb->insert( $p . 'lsv07i_abr_trainingstage', [
                ...
                'stunden'          => 2.0,
                'quelle'           => 'fitness',
                'begruendung'      => 'Fitness-Training (automatisch)',
                'manuell_geloescht'=> 0,
            ], [ '%d', '%s', '%s', '%f', '%s', '%s', '%d' ] );
        }
    ...
    public static function anw_mark_ausfall() {
        ...
        $wpdb->query( $wpdb->prepare(
            "INSERT INTO {$p}lsv07i_fit_anwesenheit (gruppe_id, slot_id, training_datum, ausgefallen, ausfall_grund, erstellt_von)
             VALUES (%d, %d, %s, %d, %s, %d)
    ...
    public static function anw_get_sessions() {
        LSV07I_Access::check( 'fit_anw_read' );
        global $wpdb;
        $p         = $wpdb->prefix;
        $gruppe_id = absint( $_POST['gruppe_id'] ?? 0 );
